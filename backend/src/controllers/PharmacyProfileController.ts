...
import {PharmacyProfileService} from "../services/";
import { handleControllerError } from "../errors";
import { HttpStatus } from "../enums/ResponseCodes";

export default class PharmacyProfileController {
  private service = new PharmacyProfileService();
  ...
  async create(req: Request, res: Response) {
    try {
      ...
      res.status(HttpStatus.CREATED).json({
        success: true,
        message: "Pharmacy profile created",
        data: profile,
  ...
  async getById(req: Request, res: Response) {
    try {
      ...
      if (!profile) {
        return res.status(HttpStatus.NOT_FOUND).json({ message: "Pharmacy profile not found" });
      }
