...
const SingleItem = ({ item }) => {
  ...
  const handleRemoveFromWishlist = () => {
    dispatch(removeItemFromWishlist(item.id));4
    showSnackbar(`${item.title} removed from Wishlist!`, 'error');
  };
    ...
    <div className="flex items-center border-t border-gray-3 py-5 px-10">
      ...
      <div className="min-w-[387px]">
        <div className="flex items-center justify-between gap-5">
          <div className="w-full flex items-center gap-5.5">
            ...
            <div>
              <h3 className="text-dark ease-out duration-200 hover:text-orange">
                <a href="#"> {item.title} </a>
              </h3>
            </div>
          </div>
      ...
      <div className="min-w-[150px] flex justify-end">
        <button
          ...
          className="inline-flex text-dark hover:text-white bg-gray-1 border border-gray-3 py-2.5 px-6 rounded-md ease-out duration-200 hover:bg-orange-dark active:bg-orange-600 hover:border-gray-3"
        >
          Add to Cart
        </button>
      </div>
